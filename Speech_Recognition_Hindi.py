...
def takeCommandHindi():
    ...
    if is_connected():
        ...
        # Fallback code when offline (if using Sphinx for recognition)
        r = sr.Recognizer()
        with sr.Microphone() as source:
            print("Listening using Sphinx...")
            r.pause_threshold = 0.7
            audio = r.listen(source)
    ...
    # Process the recognized and translated audio
    aud1 = aud.lower()
    for c in string.punctuation:
        aud1 = aud1.replace(c, "")
    animate(aud1, aud)
